...
- **`{package}`** — nests each class by its proto package, e.g.
  ...
  fully-qualified name; same-package types by short name. Omit `{package}` for the
  legacy flat layout (everything in one namespace).
...
> **Migrating from the flat layout:** the old `grpc.proto` / `generated_dir` /
> `generated_namespace` keys still work (a deprecation notice points here) and keep
> the flat output. Move them under `grpc.server` to opt into the package layout.
